...
# ---------------------------------------------------------------------------- #
    ...
    padding=0.01 #边界边距，防止发生一些计算编号溢出
    restiCoeff=0.9
    #边界处理，左
    for i in range(N):
        if x[i][0] < 0.0 + padding:
            x[i][0] = padding
            v[i][0] *= -restiCoeff#反弹

    #边界处理，右
    for i in range(N):
        if x[i][0] > 1.0 - padding:
            x[i][0] = 1.0 - padding
            v[i][0] *= -restiCoeff#反弹
    
    #边界处理，下
    for i in range(N):
        if x[i][1] < 0.0 + padding:
            x[i][1] = 0.0 + padding
            v[i][1] *= -restiCoeff #反弹 

    #边界处理，上
    for i in range(N):
        if x[i][1] > 1.0 - padding:
            x[i][1] = 1.0 - padding
            v[i][1] *= -restiCoeff #反弹 
...
while gui.running:
    ...
    # text
    gui.text(
        content=f'9/0: (-/+) Young\'s Modulus {YoungsModulus[None]:.1f}', pos=(0.6, 0.9), color=0x000000)
    gui.text(
        content=f'7/8: (-/+) Poisson\'s Ratio {PoissonsRatio[None]:.3f}', pos=(0.6, 0.875), color=0x000000)
    if damping_toggle[None]:
        gui.text(
            content='D: Damping On', pos=(0.6, 0.85), color=0x000000)
    else:
        gui.text(
